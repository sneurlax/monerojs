...
class walletRPC {
   ...
   * If multiple destinations are required, use the object/dictionary (bottom) format and pass an array of objects containing recipient
   * addresses and amount in the destinations field, like destinations: [{amount: 1, address: ...}, {amount: 2, address: ...}]
   *
   * @function transfer
   ...
   * }
   */
  transfer(amount, address = undefined, payment_id = undefined, get_tx_key = false, mixin = 10, account_index = 0, subaddr_indices = undefined, priority = 2, unlock_time = undefined, do_not_relay = false, get_tx_hex = false, get_tx_metadata = false, ring_size = 11) {
    return new Promise((resolve, reject) => {
      let params = {};
      let destinations = [];
   ...
   * @param {object} params - Array containing any of the options listed above, where only amount and address or a destionations array are required
   */
  transfer_split(amount, address = undefined, payment_id = undefined, get_tx_keys = false, mixin = 10, account_index = 0, subaddr_indices = undefined, priority = 2, unlock_time = undefined, do_not_relay = false, get_tx_hex = false, new_algorithm = false, get_tx_metadata = false, ring_size = 11) {
    return new Promise((resolve, reject) => {
      let params = {};
      let destinations = [];
   ...
   * @param {string} export_raw - Set of unsigned tx returned by "transfer" or "transfer_split" methods.
   * @param {boolean} description - If true, return the raw transaction data (optional)
   *
   * @returns {promise} resolve, reject
   * @resolves {object} - Example: {
   *   signed_txset: '',
   ...
   * }
   */
  sign_transfer(unsigned_txset, export_raw = false) {
    return new Promise((resolve, reject) => {
      let params = {
      ...
      this._run('sign_transfer', params)
      .then(result => {
        this.store() // Save wallet state after untagging accounts
        .then(() => {
          resolve(result);
  ...
  submit_transfer(tx_data_hex) {
    return new Promise((resolve, reject) => {
      ...
      this._run('submit_transfer', params)
      .then(result => {
        this.store() // Save wallet state after untagging accounts
        .then(() => {
          resolve(result);
  ...
  get_transfers(input_types = ['all'], account_index = 0, subaddr_indices = undefined, min_height = 0, max_height = 4206931337) {
    ...
    } else if (typeof input_types == 'object' && input_types.constructor === Array) {
      params['account_index'] = account_index;

      if (subaddr_indices != undefined) {
        params['subaddr_indices'] = subaddr_indices;
      }
    }
